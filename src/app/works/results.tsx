...
export default function Results() {
  ...
  const buttons = [
    {
      title: t('works.results.download'),
  ...
  return (
    <View className="py-safe flex-1 bg-white px-6 dark:bg-dark-background">
      <ScreenHeader
        title={t('works.results.title')}
        onClose={router.back}
      ...
      <BeforeAfterSlider
        ...
        width={width - 48}
        height={height * 0.7}
        beforeText={t('works.results.before')}
        afterText={t('works.results.after')}
      />
      <View className="mt-auto flex-row items-center justify-between">
        {buttons.map((item) => (
          <View key={item.title} className="items-center gap-2">
            <Pressable
              className="items-center justify-center rounded-full bg-gray-200 p-5"
